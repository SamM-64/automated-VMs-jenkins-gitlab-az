...
sudo certbot certonly --standalone -d http://vm-jenkins-ecf.westeurope.cloudapp.azure.com

# Configurer Jenkins pour utiliser le certificat
sudo sed -i 's|HTTP_PORT=8080|HTTP_PORT=-1|g' /etc/default/jenkins
sudo sed -i 's|JENKINS_HTTPS_KEYSTORE=.*|JENKINS_HTTPS_KEYSTORE="/etc/letsencrypt/live/vm-jenkins-ecf.westeurope.cloudapp.azure.com/fullchain.pem"|g' /etc/default/jenkins
